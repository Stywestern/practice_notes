...
int main()
{
    ...
    while (getline(finTest, line)) {
        ...
        double elementsum1 = 0;
        double numpeople1 = 0;

        for (auto& [usid, rat] : utilityMatrix[T_userID]) {
            elementsum1 += rat;
            numpeople1++;
        }
        double userAvarageTest = elementsum1 / numpeople1;
        ...
        for (int& movieID : movieIDs) {
            ...
            double elementsum2 = 0;
            double numpeople2 = 0;

            for (auto& [usid, rat] : utilityMatrix[movieID]) {
                elementsum2 += rat;
                numpeople2++;
            }
            double userAvarageTrain = elementsum2 / numpeople2;
            ...
            if (utilityMatrix[movieID].find(T_userID) != utilityMatrix[movieID].end()) {
                ...
                for (auto& [userID, rating] : utilityMatrix[T_movieID]) {

                    lengthv1 += rating * rating;

                    if (utilityMatrix[movieID].find(userID) != utilityMatrix[movieID].end()) {

                        dotProduct += (utilityMatrix[movieID][userID] - userAvarageTrain) * (utilityMatrix[T_movieID][userID] - userAvarageTest);
